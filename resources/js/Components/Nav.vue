...
<template>
	<nav
		class="lg:px-20 sm:px-8 px-2 py-4 min-h-12 max-h-12 fixed w-full z-50 flex items-center justify-between bg-green-700 shadow-lg"
	>
			...
			<teleport to="body">
				<Slide from="left">
					...
					>
						...
						<!-- Lista -->
						<section
							class="w-full h-full mt-2 overflow-y-auto border-y-2 border-y-green-800"
						>
							<CollapseGroup>
								{{ $t('nav.softwares') }}
								<template #children>
		...
		<div class="flex items-center gap-2">
			<template v-if="$page.props.auth.user">
				<Dropdown size="md" :content-classes="['w-full']">
					...
					<template #content>
						<div
							class="pl-4 py-2 text-sm bg-slate-300 dark:bg-slate-500 lg:hidden"
						>
							{{ $page.props.auth.user.show_name }}
						</div>
						<DropdownGroup>
							<DropdownLink
								v-if="$page.props.can.view_dashboard"

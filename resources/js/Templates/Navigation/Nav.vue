...
import CollapseGroup from '@/Components/Container/CollapseGroup.vue';
...
<template>
	<nav class="lg:px-20 sm:px-8 px-2 py-4 min-h-12 max-h-12 fixed w-full z-50 flex items-center justify-between bg-green-700 shadow-lg">
		<div class="flex items-center gap-4">
			<!-- Navegação lateral (Dispositivos móveis) -->
			<font-awesome-icon icon="bars"
				class="max-lg:inline-block hidden text-green-200"
				@click="menuOpen = true"
			/>
			<teleport to="body">
				<Slide from="left">
					...
					>
						...
						<section class="w-full h-full mt-2 overflow-y-auto border-y-2 border-y-green-800">
							<CollapseGroup>
								{{ $t('nav.softwares') }}
								<template #children>
									<CollapseGroup>Item 1</CollapseGroup>
								</template>
							</CollapseGroup>
			...
			<div class="ml-2 hidden lg:flex gap-4">
				<Dropdown align="left">
					...
					<template #content>Software</template>
				</Dropdown>
				<Dropdown align="left">
					<template #trigger>
						<NavLink>
					...
					<template #content>Games</template>
				</Dropdown>
				<Dropdown align="left">
					<template #trigger>
						<NavLink>
					...
					<template #content>Online Tools</template>
				</Dropdown>
				<NavLink href="/">
					{{ $t('nav.blog') }}
				</NavLink>
				<NavLink :href="route('store.index')">
					{{ $t('nav.store') }}
				</NavLink>
			</div>
		</div>
		...
		<div class="flex items-center gap-2">
			<template v-if="$page.props.auth.user">
				<Dropdown size="md" :content-classes="[ 'w-full' ]">
					...
					<template #content>
						...
						<DropdownGroup>
							<DropdownLink v-if="$page.props.can.view_dashboard"
			...
			<template v-else>
				<Button v-if="$page.props.canLogin"
					:href="route('login')"
				...
				<Button v-if="$page.props.canRegister"
					:href="route('register')"
					icon="arrow-right-to-bracket"
					color="secondary"
